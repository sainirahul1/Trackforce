...
const IntelligenceSuite = () => {
  ...
  const taskLogs = [
    { id: 101, title: 'Inventory Stock Check', client: 'City Mart', status: 'Completed', date: '10:30 AM', brief: 'Verified warehouse levels against digital records.' },
    { id: 102, title: 'Shelf Visibility Audit', client: 'Grand Plaza', status: 'Ongoing', date: '01:15 PM', brief: 'Monitoring brand shelf share and competitor placement.' },
    { id: 103, title: 'Promotional Setup', client: 'Metro Store', status: 'Pending', date: '04:00 PM', brief: 'Installing seasonal marketing banners.' },
    { id: 104, title: 'Safety Inspection', client: 'Quick Shop', status: 'Rejected', date: 'Yesterday', brief: 'Compliance check for outlet storage safety.' },
  ];

  // SECTOR 5: CHART CONFIGURATIONS (SPARKINES)
  const chartOptions = {
      ...
      <div className="bg-[#16191f] border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
        ...
        <div className="divide-y divide-slate-800/50">
          {employees.map((exec) => (
            <div 
              key={exec.id}
              onClick={() => { setSelectedEmployee(exec); setView('profile'); }}
            ...
            >
              ...
              <div className="flex justify-end">
                <span className="text-[10px] font-black uppercase text-blue-500 group-hover:text-blue-400 group-hover:underline tracking-widest">Profile</span>
              </div>
            </div>
          ))}
        </div>
  ...
  // SECTOR 7: PROFILE VIEW COMPONENT
  const ProfileView = () => {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-[#0b0d11] p-4 transition-colors duration-500">
        ...
        <div className="bg-white dark:bg-[#16191f] border-2 border-transparent hover:border-blue-500 hover:shadow-[0_0_20px_rgba(59,130,246,0.4)] p-6 rounded-[2rem] shadow-sm flex flex-col md:flex-row items-center justify-between mb-4 transition-all duration-300 group">
          ...
          <div className="flex items-center gap-6 mt-4 md:mt-0 pr-2">
            ...
            <div className="flex flex-col border-l-2 border-slate-100 dark:border-slate-800 pl-6">
              <h4 className="text-3xl font-black text-blue-600 italic">{successPercentage}%</h4>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
          {[
            ...
            <div key={i} className="bg-white dark:bg-[#16191f] p-5 rounded-[1.5rem] shadow-sm text-center border-2 border-transparent hover:border-blue-500 transition-all">
              ...
              <p className="text-[10px] font-black text-slate-500 dark:text-slate-300 uppercase mt-2 tracking-wide">{stat.label}</p>
            </div>
          ))}
        </div>

        <div className="bg-white dark:bg-[#16191f] border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-6 pb-4 border-b dark:border-slate-800/50 gap-4">
            <h3 className="text-[12px] font-black uppercase tracking-[0.25em] text-blue-600">Task Log Details</h3>
            <div className="flex bg-slate-50 dark:bg-slate-900 p-1.5 rounded-xl gap-1">
              {['All', 'Completed', 'Ongoing', 'Pending', 'Rejected'].map(s => (
                <button key={s} onClick={() => setStatusFilter(s)} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${statusFilter === s ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-blue-500'}`}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div className="space-y-4">
            {taskLogs.filter(t => statusFilter === 'All' || t.status === statusFilter).map(log => (
              <div key={log.id} className="group bg-slate-50 dark:bg-[#1a1d23] p-5 rounded-[1.5rem] border-2 border-transparent hover:border-blue-500 transition-all">
                <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <div className="p-2.5 bg-blue-600 text-white rounded-xl shrink-0"><ClipboardList size={16}/></div>
                      <h4 className="font-black text-slate-900 dark:text-white text-lg truncate">{log.title}</h4>
                    </div>
                    <div className="flex items-center gap-2 text-slate-500 text-[10px] font-black uppercase ml-12 mb-3">
                  ...
                  <div className="flex md:flex-col items-center md:items-end gap-3 shrink-0 w-full md:w-auto">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-2 px-6 py-2.5 bg-slate-900 dark:bg-blue-600 text-white text-[10px] font-black uppercase rounded-xl shadow-lg hover:scale-105 transition-all"
                    >
